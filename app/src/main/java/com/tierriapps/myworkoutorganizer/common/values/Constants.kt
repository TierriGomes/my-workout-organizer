package com.tierriapps.myworkoutorganizer.common.values

object Constants {
    const val PENDING_WORKOUTS_KEY: String = "pending_workouts_keys"
    var USER_ID = ""
    var USER_NAME = ""

    const val DATABASE_NAME = "local_database"
    const val PREFERENCES_NAME = "local_preferences"
    const val WORKOUT_REMOTE_COLLECTION = "workouts"

    const val ACTUAL_WORKOUT_KEY = "actual_workout"

    const val NOTIFICATION_DO_TRAINING_CHANNEL = "foreground_training_channel"
    const val NOTIFICATION_DO_TRAINING_CHANNEL_NAME = "Foreground training notification"
}